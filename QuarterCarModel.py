...
class CarView():
    def __init__(self, args):
        ...
        # assuming layout_horizontal_main is defined or passed correctly
        self.layout_horizontal_main = self.display_widgets[-1]  # Ensure this is correct

        # Creating the figure and canvas for position plot
        ...
        self.tabWidget.addTab(self.tab1, "Position vs Time")
        self.tabWidget.addTab(self.tab2, "Force vs Time")

        self.layout1 = qtw.QVBoxLayout()
        self.layout2 = qtw.QVBoxLayout()
        self.tab1.setLayout(self.layout1)
        self.tab2.setLayout(self.layout2)

        self.layout1.addWidget(NavigationToolbar2QT(self.canvas, self.tab1))
        self.layout1.addWidget(self.canvas)

        # Create a new figure for forces and add it to layout2
        ...
        self.layout2.addWidget(NavigationToolbar2QT(self.canvas2, self.tab2))
        self.layout2.addWidget(self.canvas2)

        # Add tabWidget to the main layout
        self.layout_horizontal_main.addWidget(self.tabWidget)
    ...
    def onTabChange(self, index):
        # Method to update plots based on selected tab
        self.doPlot(self.model)
    ...
    def doPlot(self, model=None):
        ...
        if self.tabWidget.currentIndex() == 0:
            ...
            if self.chk_ShowAccel.isChecked():
                ax2.plot(t, accel, 'g-', label='Body Acceleration')
                ax2.axhline(y=accel.max(), color='orange')  # horizontal line at accel.max()
                ax2.set_yscale('log' if self.chk_LogAccel.isChecked() else 'linear')
...
class CarController():
    ...
    def OptimizeSuspension(self):
        """
        Optimizes the suspension settings based on GUI inputs.
        Step 1: Set parameters based on GUI inputs by calling self.calculate(doCalc=False).
        Step 2: Make an initial guess for k1, c1, k2, ensuring they are within realistic operating ranges.
        Step 3: Optimize the suspension settings using the 'minimize' function from scipy.optimize.
        """
        # Step 1: Set parameters and update the model with GUI inputs
        self.calculate(doCalc=False)
        self.model.k1 = max(self.model.mink1, min(self.model.maxk1, self.model.k1 * 1.1))
        self.model.k2 = max(self.model.mink2, min(self.model.maxk2, self.model.k2 * 1.1))

        # Step 2: Make an initial guess for k1, c1, k2 within acceptable ranges
        x0 = np.array([
            max(self.model.mink1, min(self.model.maxk1, self.model.k1 * 1.1)),
            self.model.c1,
            max(self.model.mink2, min(self.model.maxk2, self.model.k2 * 1.1))
        ])
        ...
        # Refresh the view with updated model parameters
        self.view.updateView(self.model)
